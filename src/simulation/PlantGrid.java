...
public class PlantGrid {
	...
	public class PlantBox {
		public static final int DEAD_TIMER_NATURAL = 200;
		public static final int DEAD_TIMER_DESTROYED = 5000;
		...
		public void initTimer(boolean gotDestroyed) {
			if (gotDestroyed) {
				this.stillDeadTime = DEAD_TIMER_DESTROYED;
			} else {
				this.stillDeadTime = DEAD_TIMER_NATURAL;
			}
			this.growth = Plant.BASE_GROW_TIME;
		}
	...
	public void removePlant(int x, int y, boolean gotDestroyed) {
		if (grid[x][y].plant != null) {
			grid[x][y].deletePlant(gotDestroyed);
