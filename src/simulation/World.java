...
public class World {
	...
	public static final double SIZE = 4000;
	public static final int NUMBER_OF_STARTING_CREATURES = 100;
	public static final int NUMBER_OF_STARTING_PLANTS = 3000;
	...
	public synchronized void step() {
		...
		AnalyseStrg.getSpeedAnalyser().addData("Step.Move", second);
		//Collision
		...
		for (int i = 0; i < creatures.size(); i++) {
			...
			AnalyseStrg.getSpeedAnalyser().addData("Step.Collision/creature.Creatures", second);
			...
			for (int j = 0; j < cadavers.size(); j++) {
				...
				if (b1.inRangeOf(cad, b1.getRadius()+cad.getRadius())) {
					if (creatures.get(i).eats() && creatures.get(i).getBody().getCarnivore_eff() > 0) {
			...
			second = System.currentTimeMillis()-timestamp;
			AnalyseStrg.getSpeedAnalyser().addData("Step.Collision/creature.Cadavers", second);
			...
			second = System.currentTimeMillis()-timestamp;
			AnalyseStrg.getSpeedAnalyser().addData("Step.Collision/creature.WorldBounds", second);
			// ROCK SYSTEM
			timestamp = System.currentTimeMillis();
			...
			second = System.currentTimeMillis()-timestamp;
			AnalyseStrg.getSpeedAnalyser().addData("Step.Collision/creature.RockSystem", second);
			...
			for (int w = upLeft.getX(); w <= downRight.getX(); w++) {
				for (int h = upLeft.getY(); h <= downRight.getY(); h++) {
					...
					if (p != null && b1.edgeDistanceTo(p) < 0) {
						...
						if (creatures.get(i).eats() && creatures.get(i).getBody().getHerbivore_eff() > 0) {
							plantGrid.removePlant(w,h, true);
			...
			second = System.currentTimeMillis()-timestamp;
			AnalyseStrg.getSpeedAnalyser().addData("Step.Collision/creature.Plants", second);
		}
		long second2 = System.currentTimeMillis()-timestamp2;
		AnalyseStrg.getSpeedAnalyser().addData("Step.Collision", second2);
		timestamp = System.currentTimeMillis();
		//attacking
		for (int i = 0; i < creatures.size(); i++) {
					...
					if (i != j) {
						...
						if (projectionScalar >= 0) {
							...
							if (distance < attacked.getBody().getRadius()) {
								...
								if (creatures.get(i).getBody().getCarnivore_eff()>0) {
		...
		second = System.currentTimeMillis()-timestamp;
		AnalyseStrg.getSpeedAnalyser().addData("Step.Attacking", second);
		...
		second = System.currentTimeMillis()-timestamp;
		AnalyseStrg.getSpeedAnalyser().addData("Step.Cadavers", second);
		timestamp = System.currentTimeMillis();
		plantGrid.calculateGrowth(rockSys);
		second = System.currentTimeMillis()-timestamp;
		AnalyseStrg.getSpeedAnalyser().addData("Step.PlantGrowth", second);
		//Input & Output
		...
		for (int i = 0; i < creatures.size(); i++) {
			...
			c.workBrain(this);
			second = System.currentTimeMillis()-timestamp;
			AnalyseStrg.getSpeedAnalyser().addData("Step.InOut/creature.Brain", second);
			timestamp = System.currentTimeMillis();
			c.workBody(this);
			second = System.currentTimeMillis()-timestamp;
			AnalyseStrg.getSpeedAnalyser().addData("Step.InOut/creature.Body", second);
		}
		second2 = System.currentTimeMillis()-timestamp2;
