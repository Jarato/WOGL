...
public final class EarCut {
	 ...
	 *            ...].
	 * @param holeIndices
	 *            is an array of hole indices if any (e.g. [5, 8] for a 12-vertice
	 *            input would mean one hole with vertices 5–7 and another with
	 *            8–11).
	 * @param dim
	...
	private static void earcutLinked(Node ear, List<Integer> triangles, int dim, double minX, double minY, double size, int pass) {
		...
		if (pass == Integer.MIN_VALUE && size != Double.MIN_VALUE) indexCurve(ear, minX, minY, size);
