...
public class DrawWorld {
	...
	public static void drawRocks(Rock[] rocks) {
		for (int i = 0; i < rocks.length; i++) {
			Rock r = rocks[i];
			double[] xyPoints = r.getTrianglePoints();
			List<Integer> triangleIndices = r.getTriangleIndices();
			for (int k = 0; k < triangleIndices.size()/3; k++) {
				double[] triangleX = new double[] {xyPoints[triangleIndices.get(k*3)*2],xyPoints[triangleIndices.get(k*3+1)*2], xyPoints[triangleIndices.get(k*3+2)*2]};
				double[] triangleY = new double[] {xyPoints[triangleIndices.get(k*3)*2+1], xyPoints[triangleIndices.get(k*3+1)*2+1], xyPoints[triangleIndices.get(k*3+2)*2+1]};
				drawFilledConvexPolygon(0, 0, triangleX, triangleY, Rock.COLOR);
			}
